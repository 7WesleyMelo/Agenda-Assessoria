...
async function main() {
  ...
  for (const arquivo of arquivos) {
    ...
    await writeFile(arquivo, formatado, 'utf8');
    alterados.push(path.relative(raiz, arquivo));
  }

  if (alterados.length === 0) {
    console.log('Nenhum arquivo precisou de formatação.');
    return;
  }

  console.log('Arquivos formatados:');
